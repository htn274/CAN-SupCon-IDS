...
def get_mask(logits,threshold, num_class=10):
    """
    Produce a mask of logits based on its entropy.
    
    Parameters:
    -------------
    logits: the final output of source model
    threshold: threshold for mask
    num_class: number of classes of source model

    Output:
    mask: a binary mask indicates the weight of each sample
    """
    # The entropy of the predictions from source model
    ent = -(logits.softmax(1) * logits.log_softmax(1)).sum(1)
    # Threshodl is corelated with number of classes
...
def infer_interleave(forward_fn, inputs_train,cfg,bs):
    ...
    Parameters:
    ------------
    forward_fn: forward function to infer samples
    inputs_train: the concatenation of labeled inputs, unlabeled inputs with 
                    weak and strong argumentations
                    (inputs_l, inputs_u_w, inputs_u_s)
    cfg: settings
    bs: batch_size
    ...
    # Get the prediction of inputs_interleave
    ...
    # ret_id indicates inputs_u_s (-1) and inputs_l (-3)
    for ret_id in [-1,-3]:
        ret_list=[]
    ...
    return  logits_l,logits_u_w, logits_u_s,\
            cur_feat_l,cur_feat_u,cur_feat_s,feat_target
...
def train(epoch,train_loader , model,source_model,ema_teacher,optimizer,lr_scheduler, ssl_alg, cfg,device):
    ...
    for batch_idx, (data_l, data_u) in enumerate(train_loader):
        ...
        # forward function of target model
        ...
        if cfg.lambda_kd>0:
            ...
            with torch.no_grad():
                # This is a list of outputs produced from the end of each layer
                # in the source model
            ...
            # The features from source model
    ...
    bar.finish()
    return (losses.avg, losses_ce.avg, losses_ssl.avg, 
    losses_mmd.avg, losses_kd.avg,labeled_acc.avg, 
    unlabeled_acc.avg, masks_ssl.avg, masks_mmd.avg,masks_kd.avg)
